...
const outputDir = path.join(projectRoot, "data", "sonjj-gmail-real");
const baseUrl = "https://app.sonjj.com";
const endpointPath = "/v1/temp_gmail/list";
const pageSize = 100;
...
async function fetchAllRealGmails(apiKey) {
  const pages = [];
  const emails = [];
  const seen = new Set();
  let page = 1;
...
async function main() {
  ...
  const result = await fetchAllRealGmails(apiKey);
  const finishedAt = new Date().toISOString();
  const outputPath = path.join(outputDir, `sonjj-gmail-real-${buildTimestamp()}.json`);
  ...
  console.log(`Saved ${snapshot.count} real Gmail entries to ${outputPath}`);
}
